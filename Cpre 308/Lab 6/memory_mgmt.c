...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int i = 1;
	int km = PageFrames[0].time_of_arrival;
	for(i; i < num_frames; i++){
		if(PageFrames[i].time_of_arrival < km){
			km = PageFrames[i].time_of_arrival;
			frame_to_evict = i;
		}
	}
...
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0;
	int i = 1;
	int km = PageFrames[0].time_of_access;
	for(i; i < num_frames; i++){
		if(PageFrames[i].time_of_access < km){
			km = PageFrames[i].time_of_access;
			frame_to_evict = i;
		}
	}
	return frame_to_evict;
...
{
	int frame_to_evict = 0;
	int i = current_access + 1;
	int pageId = PageFrames[0].page_id;
	int km = 0;
	int PageWasFound = 0;

	for(i; i < num_accesses; i++){
		if(PageAccesses[i] == pageId){
			km = i;
			PageWasFound = 1;
			break;
		}
	...
	PageWasFound = 0;
	i = 1;
	int j = current_access;

	for(i; i < num_frames; i++){
		pageId = PageFrames[i].page_id;
		j = current_access + 1;

		for(j; j < num_accesses; j++){
			if(PageAccesses[j] == pageId){
				PageWasFound = 1;
				if(j > km){
					frame_to_evict = i;
					km = j;
