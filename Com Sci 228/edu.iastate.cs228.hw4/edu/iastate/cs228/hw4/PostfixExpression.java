...
public class PostfixExpression extends Expression 
{
	...
	public int evaluate() throws ExpressionFormatException, UnassignedVariableException 
    {
		...
		Scanner charScan = new Scanner(str);
		
		int value;
		
		while (charScan.hasNext())
		{
			String cur = charScan.next();
			
			for (int i = 0; i < cur.length(); i ++)
			{
				if ((!isInt(Character.toString(cur.charAt(i))) && !isOperator(cur.charAt(i)) && !isVariable(cur.charAt(i)) && cur.charAt(0) != ' ') || (cur.charAt(0) == '(' || cur.charAt(0) == ')'))
				{
		...
		while (s.hasNext())
		{
			...
			else if (isOperator(current.charAt(0)) && current.length() == 1)
			{
				...
				operandStack.push(compute(oper));
			}
	...
	private int compute(char op)  
	{
		...
		if (op == '~')
		{
			nowComputed = 0 - rightOperand;
		}
		if (op == '+')
		{
			nowComputed = leftOperand + rightOperand;
		}
		if (op == '-')
		{
			nowComputed = leftOperand - rightOperand;
		...
		if (op == '^')
		{
			nowComputed = (int) Math.pow(leftOperand, rightOperand);
		}
