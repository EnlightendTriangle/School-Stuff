 ...
 *  the interface with a heap and a heap sort algorithm.
 */
...
//TODO: O(1)
 ...
 { 
	 ...
	 int elementLastIndex = list.size() - 1;
	 int innerLastIndex = (elementLastIndex - 1) / 2;
	 return list.get(innerLastIndex);
 }
  ...
  public void trimEveryLeaf()
  {
	  ...
	  int firstNode = (list.size() - 1);
	  int firstInternalNode = (firstNode - 1) / 2;
	  
	  for(int i = firstInternalNode + 1; i <= firstNode; i++) { 
		  list.remove(firstInternalNode + 1);
	  }
  }
  
  //This getter method gets index of the parent element
  ...
  {  
	  int k = (list.size() - 2) / 2;
		 ...
		 while(k >= 0) {
			 ...
			 if(leftChild < list.size() && list.get(s).compareTo(list.get(leftChild)) > 0) {
				 
				 s = leftChild;
			 }
			 
			 if(rightChild < list.size() && list.get(s).compareTo(list.get(rightChild)) > 0) {
