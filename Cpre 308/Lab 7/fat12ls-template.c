...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for(i = 0; i < (iEntries); i = i + 1)   {
		...
		//Display filename
		toDOSName(string, buffer, offset);
		printf("%-12s\t", string);
		
		//Display Attributes
		parseAttributes(string, buffer[offset + 11]);
		printf("%s\t", string);
		
		//Display Time
		parseTime(string, endianSwap(buffer[offset + 22, buffer[offset + 23]]));
		printf("%s\t", string);
		
		//Display Date
		parseDate(string, endianSwap(buffer[offset + 24, buffer[offset + 25]]));
		printf("%s\t", string);
		
		//Display Size
		printf("%d\n", 0 /* TODO: replace 0 with actual size */);

        unsigned int last_buffer = buffer[offset+31-3] | (buffer[offset+31 - 2] << 8) | (buffer[offset+31 - 1] << 16) | (buffer[offset+31] << 24);
        printf("%d\n", last_buffer);
    }
